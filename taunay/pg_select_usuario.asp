...
'VERIFICA NÍVEL DE USUÁRIO ANTES DE GERAR A CONSULTA
if(session("nivelID") = "1")Then

	if (keyword = "") then
	 sql = "SELECT * FROM tb_usuario ORDER BY tb_usuario.usuLogin, tb_usuario.usuID ASC"
	else
	 sql = "SELECT * FROM tb_usuario WHERE usuLogin Like '%"&keyword&"%' ORDER BY tb_usuario.usuLogin, tb_usuario.usuID ASC" 
	end if

else

	if (keyword = "") then
	 sql = "SELECT * FROM tb_usuario WHERE usuID = '"&Session("usuID")&"'"
	else
	 sql = "SELECT * FROM tb_usuario WHERE usuLogin Like '%"&keyword&"%' AND usuID = '"&Session("usuID")&"'" 
	end if
...
fun = request.QueryString("fun")

if ((usuID > 0) and (fun = "desativar"))then 

Set rs02_cmd = Server.CreateObject ("ADODB.Command")
rs02_cmd.ActiveConnection = MM_conn_STRING
rs02_cmd.CommandText = "UPDATE tb_usuario set usuAtivo='N' WHERE usuID='"&usuID&"'" 
rs02_cmd.Prepared = true

Set rs02 = rs02_cmd.Execute
rs02_numRows = 0

response.Redirect("pg_select_usuario.asp")

elseif ((usuID > 0) and (fun = "ativar")) then

Set rs02_cmd = Server.CreateObject ("ADODB.Command")
rs02_cmd.ActiveConnection = MM_conn_STRING
rs02_cmd.CommandText = "UPDATE tb_usuario set usuAtivo='S' WHERE usuID='"&usuID&"'" 
rs02_cmd.Prepared = true
...
<div id="areaConteudo">
	...
	<div style="height:25px; line-height:25px; background:#ccc">Menu</div>    
	  <ul>
        ...
        <%if(Session("nivelID") = "1")Then%>
	    <li><a href="pg_insert_usuario.asp">Novo Usuário</a></li>  
        <%end if%>        
  ...
  <tr>
    ...
    <td width="751" align="center" class="titulo">USUÁRIOS</td>
    </tr>
  <tr>
    ...
    <table width="100%" border="0" align="left" cellpadding="2" cellspacing="2">
      ...
      <tr class="textoBranco">
        ...
        <td align="left" bgcolor="#9E231B">Usuario</td>
        <td width="147" align="left" bgcolor="#9E231B">Senha</td>
      ...
      <tr>
        <td align="left"><%=(rs01.Fields.Item("usuID").Value)%></td>
        <td align="left"><%=(rs01.Fields.Item("usuLogin").Value)%></td>
        <td align="left">******</td>
        <td><a href="pg_update_usuario.asp?usuID=<%=(rs01.Fields.Item("usuID").Value)%>"><img src="ico/ico_alterar.gif" width="15" height="15" border="0"/></a></td>
        <td>
        <%if rs01.Fields.Item("usuAtivo").Value="S" then%>
          <a href="pg_select_usuario.asp?fun=desativar&usuID=<%=(rs01.Fields.Item("usuID").Value)%>"><img src="ico/ico_<%=(rs01.Fields.Item("usuAtivo").Value)%>.gif" width="20" height="20" border="0" /></a>
          <%elseif rs01.Fields.Item("usuAtivo").Value="N" then %>
          <a href="pg_select_usuario.asp?fun=ativar&usuID=<%=(rs01.Fields.Item("usuID").Value)%>"><img src="ico/ico_<%=(rs01.Fields.Item("usuAtivo").Value)%>.gif" width="20" height="20" border="0" /></a>
          <%end if%>
...
%>
      <tr>
        <td colspan="5" align="left"><table border="0" align="center">
            <tr>
