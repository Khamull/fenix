...
elseif (venID <> "") then
	...
	set rs06 = server.CreateObject("adodb.recordset")
		sql06 = "SELECT tb_venda.venID, tb_venda.staID, tb_cliente.cliNome, tb_cliente.cliID, tb_cliente.cliPreferencias FROM tb_venda INNER JOIN tb_cliente ON tb_cliente.cliID = tb_venda.cliID WHERE tb_venda.venID = '"&venID&"'"
...
if (not isEmpty(Request.Form("lancar"))) then
	...
	Response.redirect("pg_insert_itemVendaInternet.asp?venID="&Request.QueryString("venID"))
...
window.location.href = "pg_insert_itemVendaInternet.asp?venID="+venID+"&itePreco="+itePreco+"&iteQtde="+iteQtde+"&tipDescr="+tipDescr+"&tipID="+tipID+"&cliID="+cliID;
...
window.location.href = "pg_insert_itemVendaInternet.asp?venID="+venID+"&itePreco="+itePreco+"&iteQtde="+iteQtde+"&tipDescr="+tipDescr+"&tipID="+tipID+"&cliID="+cliID+"&proDescricao="+proDescricao+"&proID="+proID;
...
function excluirPedido(staID)
{
	...
	{
		if(staID != "10"){
			window.location.href = "pg_insert_itemVendaInternet.asp?acao=excluirPedido&venID=<%=Request.QueryString("venID")%>";
		}else{
		window.open("pg_excluir_pedidos.asp?venID=<%=Request.QueryString("venID")%>&tipo=telefone", "Excluir", "width=300px, height=260px");
		}
	}
}	
</script>
...
<div id="areaConteudo">
  ...
  <tr>
    <td width="50" align="center"><img src="ico/ico_site.gif" alt="" width="60" height="60" border="0" class="icone" /></td>
    <td width="751" align="center" class="titulo">ITENS - VENDA -INTERNET</td>
  </tr>
   ...
   <tr>
     ...
     <table align="center" width="450">
      <tr bgcolor="#CCCC99">
       <td width="225" height="35" align="center"><input type="button" onclick="javascript: window.open('ultimos_pedidos.asp?cliID=<%=rs06.fields.item("cliID").value%>', 'Lista' , 'height = 580 , width = 375');" value="Últimos Pedidos" /></td>
       <td width="213" align="center"><input type="button" onclick="javascript: window.open('pg_tempodeentrega.asp', 'Entrega' , 'height = 180 , width = 320');" value="Tempo Médio de Entrega" /></td>
      </tr>
     </table>
   ...
   <tr>
    <td align="left" valign="top" bgcolor="#eeeeee"><font color="#ff0000">PREFERENCIAS</font></td>
    <td align="left" valign="top" bgcolor="#eeeeee"><b> <font color="#000099"><%=rs06.fields.item("cliPreferencias").value%> </font> </b></td>
   </tr>
    ...
    <form id="form1" name="form1" method="post" action="<%=Request.ServerVariables("SCRIPT_NAME")%>?venID=<%=Request.QueryString("venID")%>" onsubmit="return verForm(this)">
      <table width="777" border="0" cellpadding="1" cellspacing="1">
        ...
        <tr>
          ...
          <td height="25" align="left"><input name="itePreco" type="text" id="itePreco" onchange="calcTotal()" onblur="calcTotal()" value="<%=Replace(proPrecoVenda,",",".")%>" size="10" maxlength="10" />
&nbsp;&nbsp;Total:
<input name="iteSubTotal" type="text" id="iteSubTotal" value="<%=Replace(FormatNumber(proPrecoVenda),",",".")%>" size="10" maxlength="10" readonly="readonly" /> <input name="itePrecoCompara" type="hidden" id="itePrecoCompara" onchange="calcTotal()" onkeypress="calcTotal()" onkeydown="calcTotal()" value="<%=Replace(proPrecoVenda,",",".")%>" size="10" maxlength="10" readonly="readonly" /></td>
        </tr>
        <%
		 if Not rs04.EoF Then
		  if(rs04.fields.item("tipID").value <> "1") Then
...
%>
 <tr class="textoComum">
   ...
   <td align="left" valign="middle" bgcolor="#FFFFFF"><a href="pg_insert_itemVendaInternet.asp?venID=<%=request.querystring("venID")%>&acao=excluirItem&idItem=<%=rs07.fields.item("iteID").value%>"><img src="ico/ico_excluir.gif" width="15" height="15" border="0"/></a></td>
 </tr>
 <%
 rs07.moveNext
 loop
