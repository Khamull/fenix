...
Recordset2_cmd.CommandText = "SELECT * FROM tb_imagemhome ORDER BY imagemID ASC" 
...
sql02 = "SELECT * FROM  tb_imagemhome WHERE imagemID = '"&imagemID&"'"
...
path = Server.MapPath("upload_home")
path = path&"\"&imagem
...
sql01   = "DELETE FROM tb_imagemhome WHERE imagemID = '"&imagemID&"'"
set rs1 = conn.execute(sql01)
response.Redirect("sis_update_imagem_home.asp")

elseif(acao = "2") Then

set rs02 = Server.CreateObject("ADODB.Recordset")
sql02   = "INSERT INTO tb_imagemhome (statusID) VALUES ('"&statusID&"')"
set rs02 = conn.execute(sql02)
...
function excluir(imagemID){
	if (confirm("tem certeza que deseja excluir essa Imagem?")){
		window.location.href="sis_update_imagem_home.asp?imagemID="+imagemID+"&acao=1";
	}else{
		return false;
	}
}
 ...
 function abreJanela(imagemID){
	 window.open("sis_janela_ft_imagem_home.asp?imagemID="+imagemID, "fotos", "height = 210, width = 500");
  }
  
  function inserir(statusID){
	if (confirm("Deseja inserir uma nova foto?")){
		window.location.href="sis_update_imagem_home.asp?statusID="+statusID+"&acao=2";
	}else{
		return false;
	}
}
...
<div id="areaConteudo">
    ...
    <div style="height:25px; line-height:25px; background:#ccc"><strong>HOME - IMAGENS</strong></div>
    <div><!--<a href="sis_update_home3.asp"><input type="button" value="Quem Somos" class="botao" border="0" style="float:left;"/></a>--><a href="javascript: inserir('Novo&nbsp;titulo','Novo&nbsp;texto')"><input type="button" value="+ Nova Imagem" class="botao" border="0"/></a></div>
...
%>upload_home/<%=(Recordset2.Fields.Item("imagemCaminho").Value)%><%
else
%>img/sem_foto.png<%
