...
tipAtivo	=	Request.Form("tipAtivo")
...
'SELECIONA OS REGISTROS
		...
		sql00="SELECT "		
		sql00=sql00&" tb_tipo.tipID,"
		sql00=sql00&" tb_tipo.tipDescr,"
		sql00=sql00&" tb_tipo.tipAtivo"
		sql00=sql00&" FROM"
		sql00=sql00&" tb_tipo" 
		sql00=sql00&" WHERE"
		sql00=sql00&" tb_tipo.tipID='"&tipID&"'"
...
if (not isEmpty(Request.Form("atualizar"))) then
		...
		sql01="UPDATE tb_tipo SET"
		sql01=sql01&" tipDescr='"&tipDescr&"',"
		sql01=sql01&" usuLogin='"&usuLogin&"'," 
		sql01=sql01&" tipAtivo='"&tipAtivo&"'" 
		sql01=sql01&" WHERE" 
		sql01=sql01&" tipID='"&tipID&"'"
		set rs01 = conn.execute(sql01)
		
		'RETORNA UMA MENSAGEM DO RESULTADO DA OPERAÇÃO
		response.redirect("pg_update_tipoProduto.asp?msg=1&tipID="&tipID)
end if
%>
...
if (tipDescr.length < 3) {
	alert("Favor Informar o nome da tipo!");
...
<div id="areaConteudo">
	...
	<div style="height:25px; line-height:25px; background:#ccc">Menu</div>    
	  <ul>
	    ...
	    <li><a href="pg_select_tipoProduto.asp">Lista Tipos de Produtos</a></li>
      	<li><a href="pg_insert_tipoProduto.asp">Novo Tipo de Produto</a></li>           
  ...
  <tr>
    ...
    <td width="751" align="center" class="titulo">ATUALIZAR TIPO DE PRODUTO</td>
    </tr>
  <tr>
    <td colspan="2" align="center"><form id="form1" name="form1" method="post" action="pg_update_tipoProduto.asp?tipID=<%=tipID%>" onsubmit="return verForm(this)">
      <table width="777" border="0" cellpadding="1" cellspacing="1">
        <tr>
          <td height="25" colspan="4" align="center"><%=msg%></td>
        </tr>
        <tr>
          <td width="109" height="25" align="right">tipo:&nbsp;</td>
          <td width="267" height="25" align="left"><input name="tipDescr" type="text" id="tipDescr" value="<%=rs00.fields.item("tipDescr").value%>" size="40" maxlength="30" /></td>
        ...
        <tr>
          ...
          <td height="25" align="left"><label>
            <select name="tipAtivo" id="tipAtivo">
